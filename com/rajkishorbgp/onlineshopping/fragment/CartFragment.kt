...
class CartFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        pleaseOrderButton.setOnClickListener {
            val cartArrayListUpdate = cartDatabase.getAllDataUser()
            for (itemClass in cartArrayListUpdate) {
                homeDatabase.updateIsCart(itemClass.itemName, itemClass.isCart, R.color.cart_image_red)
            }
            Toast.makeText(
                requireActivity().applicationContext, "Your order has successfully placed",
                Toast.LENGTH_SHORT
            ).show()
            cartDatabase.clear()

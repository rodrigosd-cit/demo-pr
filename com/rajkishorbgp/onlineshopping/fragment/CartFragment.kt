...
class CartFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        pleaseOrderButton.setOnClickListener {
            val cartArrayList = cartDatabase.getAllDataUser()
            for (itemClass in cartArrayList) {
                homeDatabase.updateIsCart(itemClass.itemName, itemClass.isCart, R.color.cart_image_red)
            }
            Toast.makeText(requireActivity().applicationContext, "Your order has successfully been placed", Toast.LENGTH_SHORT).show()
            cartDatabase.clear()
